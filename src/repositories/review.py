...
class ReviewRepository:
    ...
    def get_review_by_id(self, review_id: int) -> ReviewModel | None:
    ...
    def get_review_id_by_user_id_and_product_id(self, user_id: int, product_id: int) -> ReviewModel | None:
        return (
            self.session.query(ReviewModel.id)
            .filter(ReviewModel.user_id == user_id, ReviewModel.product_id == product_id)
            .first()
        )

    # Обновляет рейтинг отзыва по его ID.
    def update_review(self, review_id: int, rating: int) -> ReviewModel | None:
