...
class OrderProductRepository:
    ...
    def remove_product_from_order(self, order_id: int, product_id: int) -> bool:
        ...
        order_product = self.session.query(self.order_product_model).filter(
            self.order_product_model.order_id == order_id,
            self.order_product_model.product_id == product_id).first()
        if order_product:
